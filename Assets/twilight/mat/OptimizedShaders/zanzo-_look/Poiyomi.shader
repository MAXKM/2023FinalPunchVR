Shader "Hidden/Locked/.poiyomi/Poiyomi 8.1/Poiyomi Toon/ad22e5383fbf6b1449976311480e2596"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			float3 applyEmission(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				emissionColor0 = hueShift(emissionColor0, frac((0.0 /*_EmissionHueShift*/) + (5.0 /*_EmissionHueShiftSpeed*/) * _Time.x) * (1.0 /*_EmissionHueShiftEnabled*/));
				#if defined(PROP_EMISSIONMASK) || !defined(OPTIMIZER_ENABLED)
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				poiLight.lightMap = 1;
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(1,0.3174468,0.3174468,1).rgb, (0.0 /*_ColorThemeIndex*/));
				poiFragData.alpha = mainTexture.a * float4(1,0.3174468,0.3174468,1).a;
				...
				if ((1.0 /*_MainHueShiftToggle*/))
				{
					...
					{
						poiFragData.baseColor = lerp(poiFragData.baseColor, hueShift(poiFragData.baseColor, shift + (5.0 /*_MainHueShiftSpeed*/) * _Time.x), hueShiftAlpha.r);
					}
					else
					{
						poiFragData.baseColor = hueShift(poiFragData.baseColor, frac((shift - (1 - hueShiftAlpha.r) + (5.0 /*_MainHueShiftSpeed*/) * _Time.x)));
					}
				}
