Shader "Hidden/Locked/.poiyomi/Poiyomi 8.1/Poiyomi Toon/5bc9ee9a1e88a4e4eafa886eda898605"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void applyMirror(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				#if defined(PROP_MIRRORTEXTURE) || !defined(OPTIMIZER_ENABLED)
				float4 mirrorTexture = POI2D_SAMPLER_PAN(_MirrorTexture, _MainTex, poiMesh.uv[(0.0 /*_MirrorTextureUV*/)], float4(0,0,0,0));
				if (inMirror)
				{
					...
					poiFragData.baseColor.rgb *= poiThemeColor(poiMods, float4(1,1,1,0).rgb, (0.0 /*_MirrorColorThemeIndex*/));
				}
				#else
				if (inMirror)
				{
